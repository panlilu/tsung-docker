...
if [[ -n "${MARATHON_URL}" ]]; then
    ...
    curl -s ${MARATHON_URL}/v2/apps/${tsung_slave_app_name} | grep -Po '"host":"\K.*?(?=")' | awk '{ print $1" tsung_slave"NR}' > /etc/tsung/tsung_slaves_hosts.conf
    ...
    curl -s ${MARATHON_URL}/v2/apps/${tsung_master_app_name} | grep -Po '"host":"\K.*?(?=")' | awk '{ print $1" tsung_master"}' > /etc/tsung/tsung_master_hosts.conf
